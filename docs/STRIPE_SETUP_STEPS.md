...
**Webhook returns 500 "Insert failed" or "Processing failed":** The webhook response body now includes a `detail` field with the database error. In Stripe Dashboard → Webhooks → click the failed event and check **Response body**. If you see e.g. `"detail": "column fulfilment_status of relation transactions does not exist"`, the `transactions` table is missing columns. In Supabase SQL Editor, run the contents of **docs/MIGRATION_transactions_all_webhook_columns.sql** (one script adds all required columns). Then Stripe will succeed on retry or on the next payment.
