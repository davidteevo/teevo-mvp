...
- **Works on localhost but not on production (e.g. app.teevohq.com) – sign-in or listings don’t load:**  
  Next.js bakes `NEXT_PUBLIC_*` env vars into the build, and the server needs `SUPABASE_SERVICE_ROLE_KEY` at runtime. In Netlify, set **all** of: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` (and `NEXT_PUBLIC_APP_URL` to your production URL, e.g. `https://app.teevohq.com`). Then trigger a **new deploy** (Deploys → Trigger deploy → Deploy site) so the build sees the vars. To confirm what’s missing, open **https://your-site-url/api/health** (e.g. `https://app.teevohq.com/api/health`) and check the response: it reports which vars are set and whether auth and listings (admin client) work.
