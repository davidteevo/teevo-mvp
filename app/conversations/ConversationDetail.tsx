...
export function ConversationDetail({ conversationId }: { conversationId: string }) {
  ...
  const FOOTER_HIDE_CLASS = "conversation-compose-focused";

  useEffect(() => {
    return () => {
      if (typeof document !== "undefined") document.body.classList.remove(FOOTER_HIDE_CLASS);
    };
  }, []);
      ...
      <div className="sticky bottom-0 shrink-0 bg-off-white-pique pt-2 pb-2 border-t border-mowing-green/20">
        ...
        <form onSubmit={handleSendMessage} className="flex gap-3 items-end">
          <input
            ...
            onFocus={() => {
              document.body.classList.add(FOOTER_HIDE_CLASS);
            ...
            onBlur={() => {
              document.body.classList.remove(FOOTER_HIDE_CLASS);
            }}
          />
          <button
            type="submit"
