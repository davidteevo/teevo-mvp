-- Teevo in-app notifications (Action Centre) + transaction timestamps for delivery confirmation / ops SLAs.

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ---------------------------------------------------------------------------
-- Transaction timestamps used by notifications + buyer confirmation
-- ---------------------------------------------------------------------------
...
COMMENT ON COLUMN public.transactions.buyer_confirmed_at IS 'Buyer explicitly confirmed delivery (Teevo order complete).';
COMMENT ON COLUMN public.transactions.delivery_issue_reported_at IS 'Buyer reported a problem instead of confirming delivery.';
COMMENT ON COLUMN public.transactions.delivered_at IS 'Carrier reported delivered (Shippo) or equivalent.';
COMMENT ON COLUMN public.transactions.label_created_at IS 'Shippo or manual shipping label became available.';
COMMENT ON COLUMN public.transactions.fulfilment_status_changed_at IS 'Last time fulfilment_status changed; used for stuck-transaction SLA.';
...
-- No INSERT/UPDATE/DELETE for authenticated clients. Writes use service role.
-- Mark-read goes through SECURITY DEFINER RPCs.
...
COMMENT ON TABLE public.notifications IS 'In-app Action Centre notifications. Read state and action state are independent.';
